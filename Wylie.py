#!/Users/yn/.virtualenvs/j2p/bin/python2.7
# -*- coding: utf-8 -*-
...
class Wylie(object):
    ...
    def fromWylieOneTsekbar(self, tokens, i):
        orig_i = i
        t = tokens[i]
        ...
        while t != None and (self.vowel(t) != None or self.consonant(t) != None) and not visarga: # STACK
    ...
    # @overloaded
    # @staticmethod
    def fromWylie(self, str_):
        return self.fromWylie(str_, None)
    ...
    def fromWylieOneStack(self, tokens, i):
        orig_i = i
        t = None
        t2 = None
        o = None
        ...
        vowel_sign = None  # any vowel signs (that go under or above the main stack)
    ...
    def toWylie(self, str_):
        return self.toWylie(str_, None, True)
    ...
    def toWylie(self, str_, warns, escape):
        out = ""
        line = 1
        units = 0
        ...
        while i < length: # ITER
            ...
            o = self.tib_other(t)
            if o != None and (t != ' ' or (escape and not self.followedByNonTibetan(str_, i))):
                out+=o
                i += 1
                units += 1
                if not escape:
            ...
            if t >= u'\u0f00' and t <= u'\u0fff':
                ...
                if self.tib_subjoined(t) != None or self.tib_vowel(t) != None or self.tib_final_wylie(t) != None:
                    self.warnl(warns, line, "Tibetan sign " + t + " needs a top symbol to attach to.")
                continue # ITER
    ...
    def handleSpaces(self, str_, i, out):
        found = 0
        orig_i = i
        while i < len(str_) and str_.charAt(i) == ' ':
            i += 1
            found += 1
    ...
    def toWylieOneTsekbar(self, str_, length, i):
        orig_i = i
        warns = []
        stacks = []
        while True: # ITER
            ...
            if st.visarga:
                break # ITER
            if i >= length or self.tib_top(str_[i]) == None:
                break # ITER
        ...
        if len(stacks) > 1 and stacks[-1].single_cons != None and self.isSuffix(stacks[-1].single_cons):
            stacks[-1].suffix = True
        if len(stacks) > 2 and stacks[-1].single_cons != None and stacks[-2].single_cons != None and self.isSuffix(stacks[-2].single_cons) and self.suff2(stacks[-1].single_cons, stacks[-2].single_cons):
            stacks[-1].suff2 = True
        ...
        if len(stacks) == 3 and stacks[0].prefix and stacks[1].suffix and stacks[2].suff2:
            ...
            if root == None:
                warns.append("Ambiguous syllable found: root consonant not known for \"" + ztr + "\".")
                root = 1
    ...
    def toWylieOneStack(self, str_, length, i):
        orig_i = i
        ffinal = None
        vowel = None
        klass = None
        ...
        while i < length:
            ...
            if o != None:
                ...
                if len(st.finals) > 0:
                    st.warns.append("Subjoined sign \"" + o + "\" found after final sign \"" + str(ffinal) + "\".")
                elif len(st.vowels) > 0:
                    st.warns.append("Subjoined sign \"" + o + "\" found after vowel sign \"" + str(vowel) + "\".")
            elif (o1) != None:
                i += 1
                st.vowels.append(o1)
                if vowel == None:
                    vowel = o1
                if len(st.finals)>0:
                    st.warns.append("Vowel sign \"" + o1 + "\" found after final sign \"" + str(ffinal) + "\".")
            elif (o2) != None:
                i += 1
                ...
                if o2 == "^":
                    ...
                    if klass in st.finals_found:
                        st.warns.append("Final sign \"" + o2 + "\" should not combine with found after final sign \"" + str(ffinal) + "\".")
                    else:
